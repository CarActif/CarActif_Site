        ...
        <div className="vehicules-list">
          ...
          {filteredVehicules.map((v) => (
            <div key={v.id} className="vehicule-card" onClick={() => navigate(`/vehicle/${v.id}`)}>
              ...
              <h4 className="vehicule-title">{v.marque} {v.modele}</h4>
              <p className="vehicule-annee">Année <b>{v.annee ? new Date(v.annee).getFullYear() : "NC"}</b></p>
              <p className="vehicule-km">Kilométrage <b>{v.kilometrage ? v.kilometrage.toLocaleString('fr-FR') : "NC"} km</b></p>
              <p className="vehicule-energy">Carburant <b>{v.energie || "NC"}</b></p>
              <p className="vehicule-boite">Boîte <b>{v.boite_vitesse || "NC"}</b></p>
              <p className="vehicule-prix">{v.prix_affiche?.toLocaleString('fr-FR')} <span className="vehicule-euro">€</span></p>
            </div>
          ))}
        </div>
      </div>
